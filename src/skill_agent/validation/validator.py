...
class StaticValidator:
    def __init__(self, policy: ValidationPolicy | None = None) -> None:
        self.policy = policy if policy is not None else ValidationPolicyLoader.default()
    ...
    def validate(self, skill: GeneratedSkill) -> ValidationReport:
        ...
        report.activation_pass = validate_skill_activation(skill, report, self.policy)
        if report.activation_pass:
            report.activation_pass = validate_skill_test_cases(skill, report)
        report.code_safety_pass = validate_code_safety(skill, report, self.policy)
